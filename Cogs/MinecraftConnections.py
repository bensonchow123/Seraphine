...
class MoreResultsButton(nextcord.ui.View):
    def __init__(self, link):
        super().__init__()
        self.add_item(nextcord.ui.Button(label='More Results', url=link))
...
class MinecraftConnections(commands.Cog):
    ...
    async def mcinfo(self, ctx, *, user_name):
        ...
        async with ctx.typing():
            try:
                online_emoji = nextcord.utils.get(self.client.emojis, name="Online")
                ...
                file = nextcord.File(await render_skin(), 'skin.png')
                embed = nextcord.Embed(title=f"{online_emoji}Minecraft Java account info of {user_name}:",
                                       colour=0xf4c2c2,
                ...
                await ctx.send(
                    embed=nextcord.Embed(title="Account not found", timestamp=ctx.message.created_at).set_image(
    ...
    async def online(self, ctx, arg):
        if ctx.channel.id != self.bot_channel.id and not ctx.author.guild_permissions.administrator:
            ...
            return
        online_emoji = nextcord.utils.get(self.client.emojis, name="Online")
        embed_to_edit = await ctx.send(embed=nextcord.Embed(title="Searching......Please wait", color=0xffffff))
        try:
            ...
            async with ctx.typing():
                ...
                try:
                    ...
                    server_query = None
                java_embed = nextcord.Embed(title=f"{online_emoji}Status of {arg}:(Java)",
                                            description=f"Server is Online with a latency of {latency}\nSupportedVersions:\n{version}",
                                            colour=0x48a14d)
                java_embed.set_author(name="MineZone",
                                      icon_url="https://cdn.discordapp.com/attachments/850019796014858280/868516287167496242/frame_26_delay-0.01s.jpg")
            ...
            try:
                ...
                async with ctx.typing():
                    ...
                    try:
                        bedrock_latency = bedrock.latency
                    except:
                        bedrock_latency = "Unknown"
                    bedrock_embed = nextcord.Embed(title=f"{online_emoji}Status of {arg}:(Bedrock)",
                                                   description=f"Server is Online with a latency of {round(bedrock_latency * 100)}ms\nRunning on {bedrock.map}",
                                                   colour=0x48a14d)
                    bedrock_embed.add_field(name="Online",
                                            value=f"There are {bedrock.players_online} online players out of {bedrock.players_max} max players",
                                            inline=False)
                    if bedrock.motd:
                        bedrock_embed.add_field(name="Description", value=bedrock.motd)
                await embed_to_edit.edit(embed=bedrock_embed)
            except ConnectionResetError:
                await embed_to_edit.edit(embed=nextcord.Embed(
                    description="The server you chose dont allow third party fetchers",
                    colour=0xf9423a))
        except:
            await embed_to_edit.edit(embed=nextcord.Embed(
                description="The server you chose is either not online , don't exist or don't use the regular Minecraft server protocols",
                colour=0xf9423a))
        ...
        async with ctx.typing():
            ...
            query_result = (
                query_obj.cse()
                    .list(
                    q=query,
                    cx=os.getenv("GoogleCustomSearchEngine"),
                    num=5,
                )
                    .execute()
            )
    ...
    def create_google_message(self, message, color):
        return nextcord.Embed(description=message, color=color).set_author(
            name=f"Minecraft wiki result", icon_url=self.guild.icon.url
        ).set_thumbnail(
