...
from nextcord import Guild, Member, Message, Role, utils, Embed, channel
...
class AutoMod(commands.Cog):
    ...
    async def _handle_spamming_violations(self, message: Message, member: Member):
        ...
        if too_many_everyone_mentions_with_nitro or too_many_scam_links:
            m: Message = (await message.channel.history(limit=1).flatten())[0]
    ...
    async def add_mute_role_as_muted_member_rejoins(self, member: Member):
